<template>
    ...
    <div class="row" style="margin-left: 0.1%; width: 98.5%;">
      ...
      <div class="col-lg-11" v-if="vendVer" style="padding-right: 0;">
        <div ref="pedVend" style="width: 100%; height: 600px;"></div>
      </div>
  ...
  export default {
    ...
    methods: {
      async alteraVendsB2b() {
        ...
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_IP}/graficos/pedidos-quantidade-mes-vend?setor=B2B`,
            {
              headers: {
          ...
          response.data.forEach(element => {
            const pedidos = JSON.parse(element.PEDIDOS).map(item => item.total_pedidos);
      ...
      async alteraVendsB2c() {
        ...
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_IP}/graficos/pedidos-quantidade-mes-vend?setor=B2C`,
            {
              headers: {
                Authorization: `jwt=${this.getCookie('jwt')}`,
              },
      ...
      async mostraVend() {
        try {
          this.mesVer = false;
          this.vendVer = true;
          this.$nextTick(() => this.pedVend()); // Inicializa o gráfico após garantir que o DOM foi atualizado
        } catch (error) {
          console.log(error)
          alert('Falha ao buscar informações. Favor tentar novamente mais tarde.');
        }
      ...
      async initChart() {
        ...
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_BACKEND_IP}/graficos/pedidos-quantidade-mes`,
            {
              headers: {
                Authorization: `jwt=${this.getCookie('jwt')}`,
              },
          ...
          const option = {
            title: {
              text: 'Quantidade de pedidos por mês',
            },
            tooltip: {},
            xAxis: {
              data: meses,
      ...
      async pedVend() {
        const chartDom = this.$refs.pedVend;
        if (!chartDom) return;
