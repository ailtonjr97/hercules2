<template>
    ...
    <table-top :resultados="resultados">
        <template v-slot:tableButtons>
            ...
            <router-link class="button-8" to="/comercial/cotacao-de-frete">Abertas</router-link>
        </template>
    </table-top>
    <div class="row mb-2">
...
export default{
    ...
    methods: {
        ...
        async openItensModal(numped, revisao){
            try {
                this.carregandoinfo = true;
                this.itensModal = true;
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_IP}/comercial/proposta-frete-itens/${numped}/${revisao}`, config);
        ...
        async pesquisa(pedido, cotador_id, results, vendedor, identificador){
            try {
                this.carregando = true;
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_IP}/comercial/proposta-de-frete-arquivadas/pesquisa?pedido=${pedido}&resultados=${results}&cotador_id=${cotador_id}&vendedor=${vendedor}&identificador=${identificador}`, config);
                this.respostas = response.data;
                this.resultados = response.data.length;
                this.carregando = false;
            } catch (error) {
        ...
        async refresh(){
            try {
                ...
                this.carregando = true;
                const response = await axios.get(`${import.meta.env.VITE_BACKEND_IP}/comercial/proposta-de-frete-arquivadas`, config);
                this.respostas = response.data;
                this.resultados = response.data.length;
                this.fullLoad = true;
                this.carregando = false;
